...
const RegistrationForm = (props) => {
  ...
  const [membershipEndDateRequired, setMembershipEndDateRequired] = React.useState(false);
  const dateFormatList = ['MM/DD/YYYY', 'MM/DD/YY','YYYY-MM-DDTHH:mm:ss.000Z'];

  let initialValues = {
    ...membershipState,
    'membershipEndDate': membershipState.membershipEndDate ? moment(membershipState.membershipEndDate, dateFormatList[0]) : null
  }
  React.useEffect(() => {
    if(props.membership){
      ...
      console.log('No membership id was provided')
    }

  }, [])

  React.useEffect(() => {
    if (membershipState.timeRange && membershipState.timeRange == 'specificDate') {
      setMembershipEndDateRequired(true)
    }
    else {
      setMembershipEndDateRequired(false)
    }
    form.resetFields();
    console.log('state after reset fields', membershipState)
  }, [membershipState])
  ...
  const onFinish = values => {
    ...
    console.log('Membership id', membershipState.id)
    setTimeout(() => {
      try {

        const path = `/admin/membership/${membershipState.id}`;
        const myInit = {
          headers: {},
          response: true,
          body: cleanedData,
          ...
          <Panel header="Enrollment Settings" key="4">
            ...
            <Form.Item
              label={
                <span>
                  End Date&nbsp;
                  <Tooltip title="The end date of this membership.">
                    <QuestionCircleOutlined />
                  </Tooltip>
                </span>}
            >
              <Form.Item 
                ...
                hidden={membershipEndDateRequired ? false : true}
                rules={[
                  {
                    required: membershipEndDateRequired,
                    message: 'Please enter the end date for your membership',
                  },
                ]}
              >
                <DatePicker
            ...
            <Form.Item
                ...
                hidden={membershipEndDateRequired}
                >
                  <Form.Item
