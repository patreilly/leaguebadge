...
const RegisterProgram = (props) => {
  ...
  const leagueid= urlParams.programid.split('-')[0];
  ...
  const getProgramData = (leagueArray, leagueIndex, programid) => {
    let programIndex = leagueArray[leagueIndex].programs.findIndex(program => program.id === programid)
    return leagueArray[leagueIndex].programs[programIndex]
  }
  ...
  React.useEffect(() => {
    if(props.profile.league_admin_info){
      let leagueIndex = props.profile.league_admin_info.findIndex(league => league.id === leagueid)
      let programData = getProgramData(props.profile.league_admin_info,leagueIndex, programid)
      // console.log('program data', programData)

      setInitialValues({
        ...programData,
        'requiredFields': programData.requiredFields ? convertRequiredFields(programData.requiredFields, true) : null,
        'programEffectiveDate': programData.programEffectiveDate ? moment(programData.programEffectiveDate, dateFormatList[0]) : null, 
        'programEndDate': programData.programEndDate ? moment(programData.programEndDate, dateFormatList[0]) : null, 
      })

      if (programData.programAgreement){
        Storage.get(
          programData.programAgreement,
          {
            level: 'public',
            download: false
          }
        ...
        .then(response => {
          ...
          // downloadBlob(response.Body, 'program-agreement')
        })
      }
    }
  }, [props.profile])
  ...
  // React.useEffect(() => {
  //   console.log('agreement doc state', agreementDocState)
  // }, [agreementDocState])

  /**
   * Returns array of required fields for the UI or the database
   * 
   * param Array data : the list of fields to convert
   * param Boolean toUiFields : direction toggle for converstion
   * returns Array
   */
  ...
  const cleanFormData = (formValues) => {
    ...
    for (const field of Object.keys(formValues)){
      if(moment.isMoment(formValues[field])){
        ...
        result[field] = convertRequiredFields(formValues.requiredFields, false)
      }
      else {
        result[field] = formValues[field]
      }
    }
    // delete values we don't persist from the form
    delete result.modified
    delete result.date_added
    delete result.id
    delete result.league //derived from url params

    return result
  }

  const handleDownloadAgreement = (s3Key) => {
    console.log(s3Key)
    try {
      Storage.get(
        s3Key,
  ...
  const handleRequiredFieldsEvent = (event) => {
    if(event.target.checked){
      if(event.target.value === 'govIdUrl'){
  ...
  const amplifyUploadAgreement = async options => {
    ...
    try {
      ...
      .then(response => {
        console.log('agreementDocState', agreementDocState)
        ...
        // console.log('aggreement doc state after deleting stuff', agreementDocState)

        const path = `/admin/program/${programid}`;
        const myInit = {
          headers: {},
          response: true,
          body: {programAgreement: response.key},
        }
        API
        ...
        <Collapse onChange={collapseCallback} collapsible="header">
          <Panel header="Enrollment Settings" key="3">
            ...
            <Form.Item
              ...
              hidden={newMemberApprovalState ? false : true}
              rules={[
                {
            ...
            <Form.Item
              ...
              hidden={renewalApprovalState ? false : true}
              rules={[
                {
            ...
            <Form.Item
              ...
              label={
                <span>
                  Program Agreement&nbsp;
                  <Tooltip title="Your legal agreement in PDF that players must agree to before joining this program.">
                    <QuestionCircleOutlined />
                  </Tooltip>
                </span>
              }>
